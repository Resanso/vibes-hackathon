#!/usr/bin/env bash
# Deploy telegram-service/ to the VPS over the `nera-vps` SSH alias (see
...
REMOTE_PATH="/root/nera/telegram-service"
PM2_APP="nera-telegram"
LOCAL_PATH="$(cd "$(dirname "${BASH_SOURCE[0]}")/../telegram-service" && pwd)"
